...
layout(location = 6) in vec4 fragPositon;
...
vec3 directionToLight = normalize(fragLight);//zmienic to na zmienialne
...
void main() 
{
    ...
    float lightIntensity = max(dot(normalWorld, directionToLight), 0.1);
